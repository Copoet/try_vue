...
<div class="tables">
  ...
  <div class="main-table">
    <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" @selection-change="tableSelectionChange" border>
      ...
      <el-table-column prop="sys_name" label="参数名称">
      </el-table-column>
      <el-table-column prop="sys_value" label="参数值">
      </el-table-column>
    ...
    <el-dialog title="参数添加" :visible.sync="addDialog">
      <el-form>
        <el-form-item label="参数名称" :label-width="formLabelWidth">
          <el-input v-model="addForm.sys_name" auto-complete="off" placeholder="请输入内容"></el-input>
        </el-form-item>
        <el-form-item label="参数值" :label-width="formLabelWidth">
          <el-input v-model="addForm.sys_value" auto-complete="off" placeholder="请输入内容"></el-input>
        </el-form-item>
        <el-form-item label="参数说明" :label-width="formLabelWidth">
          <el-input v-model="addForm.sys_explain" auto-complete="off" placeholder="请输入内容"></el-input>
        </el-form-item>
        ...
        <el-form-item label="参数类型" :label-width="formLabelWidth">
          <el-select v-model="addForm.sys_type" placeholder="请选择分组">
  ...
  <div class="edit-dialog">
    <el-dialog title="参数编辑" :visible.sync="editDialog">
      <el-form :model="editForm">
        <el-form-item label="参数名称" :label-width="formLabelWidth">
          <el-input v-model="editForm.sys_name" auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="参数值" :label-width="formLabelWidth">
          <el-input v-model="editForm.sys_value" auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="状态" :label-width="formLabelWidth">
        ...
        <el-form-item label="分组" :label-width="formLabelWidth">
          <el-select v-model="editForm.sys_type" placeholder="请选择分组">
            <el-option label="文本" value="text"></el-option>
            <el-option label="字符" value="varchar"></el-option>
            <el-option label="整形" value="int"></el-option>
          </el-select>
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      ...
      http.getSystemData(data).then(res => {
        if (res.code == 2000) {
    ...
    addData() {
      ...
      http.addSystemData(addData).then(res => {
        if (res.code == 2000) {
          this.$notify.success({
            title: 'success',
            message: res.msg,
    ...
    deleteData() {
      ...
      if (deleteObj.length == 0) {
        ...
        this.$confirm('是否删除?', '提示', {
          ...
          http.delMenu(deleteObj).then(res => {
            if (res.code == 2000) {
              this.$notify.success({
                title: 'success',
                message: res.msg,
    ...
    editData() {
      ...
      http.updateSystemData(addData).then(res => {
        if (res.code == 2000) {
          this.$notify.success({
            title: 'success',
            message: res.msg,
